...
public class StackedBaseSpawner extends com.bgsoftware.wildstacker.nms.v26_2.spawner.AbstractStackedBaseSpawner {
    ...
    protected Entity generateEntity(ServerLevel serverLevel, double x, double y, double z, boolean rotation) {
        ...
        try (ProblemReporter.ScopedCollector reporter = new ProblemReporter.ScopedCollector(this::toString, LOGGER)) {
            ValueInput input = TagValueInput.create(reporter, serverLevel.registryAccess(), entityCompound);
            return EntityType.loadEntityRecursive(input, serverLevel, EntitySpawnReason.SPAWNER, entity -> {
