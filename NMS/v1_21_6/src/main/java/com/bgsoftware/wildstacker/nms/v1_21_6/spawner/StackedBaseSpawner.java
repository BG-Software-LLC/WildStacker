...
public class StackedBaseSpawner extends BaseSpawner {
    ...
    public void serverTick(ServerLevel serverLevel, BlockPos blockPos) {
        ...
        Optional<EntityType<?>> entityTypesOptional;
        try (ProblemReporter.ScopedCollector scopedCollector = new ProblemReporter.ScopedCollector(this::toString, LOGGER)) {
            ValueInput valueInput = TagValueInput.create(scopedCollector, serverLevel.registryAccess(), entityToSpawn);
            entityTypesOptional = EntityType.by(valueInput);
        }
    ...
    private MobSpawnResult attemptMobSpawning(ServerLevel serverLevel, BlockPos blockPos, CompoundTag entityToSpawn,
        ...
        for (int i = 0; i < mobsToSpawn; i++) {
            ...
            Vec3 position = entityToSpawn.read("Pos", Vec3.CODEC).orElseGet(() -> new Vec3(
            ...
            Location location = new Location(serverLevel.getWorld(), position.x, position.y, position.z);

            if (stackedSpawner.isDebug())
                Debug.debug("StackedBaseSpawner", "attemptMobSpawning", "location=" + location);

            if (!serverLevel.noCollision(entityToSpawnType.getSpawnAABB(position.x, position.y, position.z))) {
                if (stackedSpawner.isDebug())
            ...
            try {
                ...
                if (type != null) {
                    PreSpawnerSpawnEvent event = new PreSpawnerSpawnEvent(location,
            ...
            Entity bukkitEntity = generateEntity(serverLevel, position.x, position.y, position.z, true);
    ...
    private void resetSpawnDelay(ServerLevel serverLevel, BlockPos blockPos) {
        ...
        // Set mob spawn data
        this.spawnPotentials.getRandom(serverLevel.getRandom()).ifPresent(spawnData ->
                this.setNextSpawnData(serverLevel, blockPos, spawnData));
    ...
    private boolean handleEntitySpawn(ServerLevel serverLevel, BlockPos blockPos,
        ...
        if (entity instanceof Mob mob) {
            ...
            if (entityToSpawn.size() == 1 && entityToSpawn.getString("id").isPresent()) {
