...
import static com.bgsoftware.wildstacker.nms.NMSMappings_v1_18_R1.*;
...
public final class NMSAdapter_v1_18_R1 implements NMSAdapter {
    ...
    private static final ReflectMethod<Float> GET_SOUND_VOLUME = new ReflectMethod<>(EntityLiving.class, "eu");
    ...
    public Zombie spawnZombieVillager(Villager villager) {
        ...
        if (hasCustomName(entityVillager)) {
            NMSMappings_v1_18_R1.setCustomName(entityZombieVillager, NMSMappings_v1_18_R1.getCustomName(entityVillager));
            NMSMappings_v1_18_R1.setCustomNameVisible(entityZombieVillager, getCustomNameVisible(entityVillager));
        }
        ...
        addEntity(getWorld(entityVillager), entityZombieVillager, CreatureSpawnEvent.SpawnReason.INFECTION);
        sendWorldEvent(getWorld(entityVillager), null, 1026,
                new BlockPosition(locX(entityVillager), locY(entityVillager), locZ(entityVillager)), 0);
    ...
    public void setInLove(Animals entity, Player breeder, boolean inLove) {
        ...
        if (inLove) {
            NMSMappings_v1_18_R1.setInLove(nmsEntity, entityPlayer);
        } else {
            resetLove(nmsEntity);
        }
    }
    ...
    public boolean isInLove(Animals entity) {
        return NMSMappings_v1_18_R1.isInLove((EntityAnimal) ((CraftEntity) entity).getHandle());
    }
    ...
    public boolean isAnimalFood(Animals animal, org.bukkit.inventory.ItemStack itemStack) {
        ...
        return isBreedItem(nmsEntity, CraftItemStack.asNMSCopy(itemStack));
    }
    ...
    public void setKiller(LivingEntity livingEntity, Player killer) {
        EntityLiving entityLiving = ((CraftLivingEntity) livingEntity).getHandle();
        entityLiving.bd = killer == null ? null : ((CraftPlayer) killer).getHandle();
    }
    ...
    public boolean canSpawnOn(org.bukkit.entity.Entity bukkitEntity, Location location) {
        ...
        EntityTypes<?> entityTypes = getEntityType(((CraftEntity) bukkitEntity).getHandle());
    ...
    public boolean doesStriderHaveSaddle(Strider strider) {
        return hasSaddle(((CraftStrider) strider).getHandle());
    }
    ...
    public void handleSweepingEdge(Player attacker, org.bukkit.inventory.ItemStack usedItem, LivingEntity target, double damage) {
        ...
        List<EntityLiving> nearbyEntities = getEntities(getWorld(targetLiving), EntityLiving.class,
                grow(getBoundingBox(targetLiving), 1.0D, 0.25D, 1.0D));
        ...
        for (EntityLiving nearby : nearbyEntities) {
            if (nearby != targetLiving && nearby != entityHuman && !entityHuman.r(nearby) &&
                ...
                damageEntity(nearby, playerAttack(entityHuman).sweep(), sweepDamage);
            }
        }
    }
    ...
    public String getCustomName(org.bukkit.entity.Entity entity) {
        ...
        IChatBaseComponent chatBaseComponent = NMSMappings_v1_18_R1.getCustomName(((CraftEntity) entity).getHandle());
    ...
    public void setCustomName(org.bukkit.entity.Entity entity, String name) {
        if (HEX_COLOR_PATTERN.matcher(name).find()) {
            ...
            // Much more optimized way than Bukkit's method.
            NMSMappings_v1_18_R1.setCustomName(((CraftEntity) entity).getHandle(),
    ...
    public boolean handleTotemOfUndying(LivingEntity livingEntity) {
        ...
        if (!isEmpty(totemOfUndying)) {
            ...
            broadcastEntityEffect(getWorld(entityLiving), entityLiving, (byte) 35);
        }
    ...
    public SyncedCreatureSpawner createSyncedSpawner(CreatureSpawner creatureSpawner) {
        ...
        TileEntityMobSpawner tileEntityMobSpawner = (TileEntityMobSpawner) getTileEntity(world, blockPosition);
    ...
    public org.bukkit.inventory.ItemStack getPlayerSkull(org.bukkit.inventory.ItemStack bukkitItem, String texture) {
        ...
        setString(skullOwner, "Id", new UUID(texture.hashCode(), texture.hashCode()).toString());
        ...
        NMSMappings_v1_18_R1.setTag(itemStack, nbtTagCompound);
    ...
    public void playPickupAnimation(LivingEntity livingEntity, Item item) {
        ...
        ChunkProviderServer chunkProvider = getChunkProvider((WorldServer) getWorld(entityLiving));
        ...
        broadcast(chunkProvider, entityItem, new PacketPlayOutEntityMetadata(getId(entityItem), getDataWatcher(entityItem), true));
    }
    ...
    public void playSpawnEffect(LivingEntity livingEntity) {
        doSpawnEffect((EntityInsentient) ((CraftLivingEntity) livingEntity).getHandle());
    }
    ...
    public boolean handlePiglinPickup(org.bukkit.entity.Entity bukkitPiglin, Item bukkitItem) {
        ...
        ItemStack itemStack = cloneItemStack(getItemStack(entityItem));
        setCount(itemStack, 1);
        net.minecraft.world.item.Item item = getItem(itemStack);
        if (isTagged(TagsItem.O, item)) {
            removeMemory(behaviorController, MemoryModuleType.Y);
            if (!isEmpty(NMSMappings_v1_18_R1.getItemInOffHand(entityPiglin))) {
                entityPiglin.b(entityPiglin.b(EnumHand.b));
            }

            setSlot(entityPiglin, EnumItemSlot.b, itemStack);
            entityPiglin.d(EnumItemSlot.b);

            if (item != PiglinAI.c) {
                setPersistent(entityPiglin);
            }

            behaviorController.a(MemoryModuleType.X, true, 120L);
        } else if ((item == Items.nJ || item == Items.nK) && !hasMemory(behaviorController, MemoryModuleType.ap)) {
    ...
    public boolean handleEquipmentPickup(LivingEntity livingEntity, Item bukkitItem) {
        ...
        ItemStack equipmentItem = getEquipment(entityLiving, equipmentSlotForItem);

        double equipmentDropChance = entityLiving.bU[equipmentSlotForItem.b()];
        ...
        setSlot(entityLiving, equipmentSlotForItem, itemStack);
        ...
        if (!isEmpty(itemStack) && equipSoundEffect != null) {
            ...
            playSound(entityLiving, equipSoundEffect, 1.0F, 1.0F);
        }
    ...
    public void giveExp(Player player, int amount) {
        ...
        if (!isEmpty(mendingItem) && usesDurability(getItem(mendingItem))) {
            ...
            if (!event.isCancelled()) {
                ...
                setDamage(mendingItem, getDamage(mendingItem) - repairAmount);
            }
        }
    ...
    public void updateEntity(LivingEntity sourceBukkit, LivingEntity targetBukkit) {
        ...
        saveData(source, nbtTagCompound);

        setFloat(nbtTagCompound, "Health", getMaxHealth(source));
        ...
        if (targetBukkit instanceof Zombie) {
            ...
            ((Zombie) targetBukkit).setBaby(hasKey(nbtTagCompound, "IsBaby") && getBoolean(nbtTagCompound, "IsBaby"));
        }

        loadData(target, nbtTagCompound);
    }
    ...
    public org.bukkit.inventory.ItemStack setTag(org.bukkit.inventory.ItemStack itemStack, String key, Object value) {
        ...
        if (value instanceof Boolean)
            set(tagCompound, key, NBTTagByte.a((boolean) value));
        else if (value instanceof Integer)
            set(tagCompound, key, NBTTagInt.a((int) value));
        else if (value instanceof String)
            setString(tagCompound, key, (String) value);
        else if (value instanceof Double)
            set(tagCompound, key, NBTTagDouble.a((double) value));
        else if (value instanceof Short)
            set(tagCompound, key, NBTTagShort.a((short) value));
        else if (value instanceof Byte)
            set(tagCompound, key, NBTTagByte.a((byte) value));
        else if (value instanceof Float)
            setFloat(tagCompound, key, (float) value);
        else if (value instanceof Long)
            set(tagCompound, key, NBTTagLong.a((long) value));
    ...
    public <T> T getTag(org.bukkit.inventory.ItemStack itemStack, String key, Class<T> valueType, Object def) {
        ...
        NBTTagCompound tagCompound = hasTag(nmsItem) ? NMSMappings_v1_18_R1.getTag(nmsItem) : new NBTTagCompound();

        if (tagCompound != null) {
            if (!hasKey(tagCompound, key))
                return valueType.cast(def);
    ...
    public void loadEntity(StackedEntity stackedEntity) {
        ...
        if (dataContainer.has(STACK_AMOUNT, PersistentDataType.INTEGER)) {
            ...
            for (String scoreboardTag : getScoreboardTags(entityLiving)) {
    ...
    public void loadItem(StackedItem stackedItem) {
        ...
        if (dataContainer.has(STACK_AMOUNT, PersistentDataType.INTEGER)) {
            ...
            // Old saving method
            EntityItem entityItem = (EntityItem) ((CraftItem) item).getHandle();
            for (String scoreboardTag : getScoreboardTags(entityItem)) {
                if (scoreboardTag.startsWith("ws:")) {
                    String[] tagSections = scoreboardTag.split("=");
                    if (tagSections.length == 2) {
                        try {
    ...
    private static class SyncedCreatureSpawnerImpl extends CraftBlockEntityState<TileEntityMobSpawner> implements SyncedCreatureSpawner {
        ...
        SyncedCreatureSpawnerImpl(org.bukkit.World bukkitWorld, TileEntityMobSpawner tileEntityMobSpawner) {
            ...
            blockPosition = NMSMappings_v1_18_R1.getPosition(tileEntityMobSpawner);
            blockLocation = new Location(bukkitWorld, NMSMappings_v1_18_R1.getX(blockPosition),
                    NMSMappings_v1_18_R1.getY(blockPosition), NMSMappings_v1_18_R1.getZ(blockPosition));
        }
        ...
        public EntityType getSpawnedType() {
            try {
                ...
                EntityType entityType = key == null ? EntityType.PIG : EntityType.fromName(getKey(key));
        ...
        public void setSpawnedType(EntityType entityType) {
            if (entityType != null && entityType.getName() != null) {
                setMobName(NMSMappings_v1_18_R1.getSpawner(getSpawner()),
        ...
        public String getCreatureTypeName() {
            MinecraftKey key = getMobName();
            return key == null ? "PIG" : getKey(key);
        }
        ...
        TileEntityMobSpawner getSpawner() {
            return (TileEntityMobSpawner) NMSMappings_v1_18_R1.getTileEntity(world, blockPosition);
        }

        private MinecraftKey getMobName() {
            MobSpawnerData mobSpawnerData = NMSMappings_v1_18_R1.getSpawner(getSpawner()).e;
            String id = getString(getEntity(mobSpawnerData), "id");
